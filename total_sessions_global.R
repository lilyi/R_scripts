...
myfunction_top3 <- function(stime, etime){
  ...
  write.csv(gaData, paste(tit, "global",".csv", sep="")) 
...
TOP3_IDs <- myfunction_top3(stime, etime)
result_TOP3 <- data.frame(sapply(TOP3_IDs, as.character), stringsAsFactors=FALSE)
coname <- rep(c("ID","type","sessions","model"),3)
colnames(result_TOP3) <- coname
write.csv(result_TOP3, paste(tit,"/top3.csv", sep="")) 
t_top3 <- t(result_TOP3)

###################################################################
CLIST[1:12] <- paste(clist[1],coname,sep=".")
