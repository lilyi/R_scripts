...
row.names(res) <- c(1:3)
S <- rep('sessions', 20)
SS <- lapply(lan_list, paste, 'sessions', sep="_")
A <- lapply(lan_list, paste, 'page', sep="_")
AA <- mapply(append, SS, A, SIMPLIFY=TRUE)
colnames(res) <- AA
View(res)
write.csv(res, "solution_top3_2.csv") 
