...
compare_biweek <- function(cname){
  ...
  lastdata <- read.csv(paste(last_date, '/total_dat/', cname, '.csv', sep=""), header = T)
...
write.csv(B, paste(this_date, '/compare.csv', sep=""))
# cname <- "Germany"
biweek_sesseions <- function(cname){
  ...
  p <- ggplot(mdat, aes(date, sessions, colour = biweek))+#, linetype=country)) + 
    ...
    labs(title = paste(cname), x = paste("date (this, comparing with", last_date, ")", sep=""), y = "sessions")
    #xlim(1,12)
  ggsave(paste(this_date, "/total_compare/", cname,".png",sep=""))
  return()
}
