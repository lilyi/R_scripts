...
compare_biweek <- function(cname){
  thisdata <- read.csv(paste('0417-0428/total_dat/', cname, '.csv', sep=""), header = T)
  maxi_T <- max(thisdata$sessions)
  mini_T <- min(thisdata$sessions)
  ave_T <- round(mean(thisdata$sessions))
  sum_T <- sum(thisdata$sessions)
  lastdata <- read.csv(paste('0403-0414/total_dat/', cname, '.csv', sep=""), header = T)
...
write.csv(B, '0417-0428/compare.csv')
cname <- "Germany"
biweek_sesseions <- function(cname){
  ...
  p <- ggplot(mdat, aes(date, sessions, colour = biweek))+#, linetype=country)) + 
    ...
    labs(title = paste(cname), x = "date (this, comparing with 0403-0414)", y = "sessions")
    #xlim(1,12)
  ggsave(paste("0417-0428/total_compare/", cname,".png",sep=""))
  return()
}
